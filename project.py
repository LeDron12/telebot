...
def db_table_val(user_id: int, vk_public: str, twitter_public: str,
	...
	cursor = conn.cursor()
	cursor.execute('INSERT INTO all_users (user_id, vk_public, twitter_public, last_post_vk, last_post_twitter) VALUES (?, ?, ?, ?, ?)',
	...
	for wall_post in wall_posts:
		if wall_post["id"] not in last_posts:
			if "attachments" in wall_post:
				...
				descriptions.append(wall_post["text"])
				photo_urls.append([])
				photo_urls[-1].append("https://sun9-30.userapi.com/impf/c834202/v834202747/242f8/CS8oRxwfjos.jpg?size=1197x801&quality=96&sign=691d41fd9b022093980e3b6140cb4737&type=album")
				last_posts_new.append(wall_post["id"])
		else:
...
def hello(message):
	if db_check(message.from_user.id):
		bot.send_message(message.chat.id, "Добрый день! Этот бот является альтернативой ленты в Вк. Напиши /help, если хочешь узнать больше!")
...
def help(message):
	...
	bot.send_message(message.chat.id, "Добавить паблик VK: /add_vk 'ссылка на паблик' (пабликов можно добавить не больше 15, паблик не должен быть закрытым)" + '\n' +
